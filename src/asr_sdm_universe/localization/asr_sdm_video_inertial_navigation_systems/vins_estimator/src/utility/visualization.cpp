...
void registerPub(rclcpp::Node::SharedPtr n)
{
  ...
  pub_camera_pose_visual =
    n->create_publisher<visualization_msgs::msg::MarkerArray>("camera_pose_visual", 1000);
...
void pubLatestOdometry(
  const Eigen::Vector3d & P, const Eigen::Quaterniond & Q, const Eigen::Vector3d & V,
...
void printStatistics(const Estimator & estimator, double t)
{
  ...
  printf(
    "position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(),
  ...
  if (ESTIMATE_TD) RCUTILS_LOG_INFO("td %f", estimator.td);
}
...
void pubTF(const Estimator & estimator, const std_msgs::msg::Header & header)
{
  ...
  // world->base is owned by odom_visualization / the robot TF tree.
  // Publishing it here would give `base` two parents.

  transform_cam.header.stamp = header.stamp;
  transform_cam.header.frame_id = "base";
