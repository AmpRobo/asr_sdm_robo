...
// Planning debug visualization implementation.
...
PlanningVisualization::PlanningVisualization(const std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  traj_pub_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("/planning_vis/trajectory", 20);
  pubs_.push_back(traj_pub_);

  topo_pub_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("/planning_vis/topo_path", 20);
  pubs_.push_back(topo_pub_);

  predict_pub_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("/planning_vis/prediction", 20);
  pubs_.push_back(predict_pub_);

  visib_pub_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("/planning_vis/visib_constraint", 20);
  pubs_.push_back(visib_pub_);

  frontier_pub_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("/planning_vis/frontier", 20);
...
//                                           const Eigen::Vector4d& color2, const Eigen::Vector4d&
//                                           color3, int id) {
  ...
  vector<vector<Eigen::Vector3d>> & paths, double line_width)
{
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    ...
    displayLineList(
      edge_pt1, edge_pt2, line_width, getColor(double(i) / (last_topo_path1_num_)),
      SELECT_PATH + i % 100, 1);
  }
}
  ...
  vector<vector<Eigen::Vector3d>> & paths, double size)
{
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    ...
    displayLineList(
      edge_pt1, edge_pt2, size, getColor(double(i) / (last_topo_path2_num_), 0.2),
