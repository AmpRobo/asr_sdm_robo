...
void TopologyPRM::init(const std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  node_->declare_parameter("topo_prm.sample_inflate_y", -1.0);
  ...
  node_->declare_parameter("topo_prm.max_sample_time", -1.0);
  ...
  max_sample_time_ = node_->get_parameter("topo_prm.max_sample_time").as_double();
...
void TopologyPRM::findTopoPaths(
  Eigen::Vector3d start, Eigen::Vector3d end, vector<Eigen::Vector3d> start_pts,
...
bool TopologyPRM::lineVisib(
  const Eigen::Vector3d & p1, const Eigen::Vector3d & p2, double thresh, Eigen::Vector3d & pc,
  int caster_id)
{
  ...
  while (casters_[caster_id].step(ray_pt)) {
    ...
    dist = edt_environment_->esdf_map_->getDistance(pt_id);
    if (dist <= thresh) {
      edt_environment_->esdf_map_->indexToPos(pt_id, pc);
      return false;
    }
  }
  return true;
