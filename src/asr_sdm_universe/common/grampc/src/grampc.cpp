...
namespace grampc
{
    ...
    void Grampc::check_gradients(ctypeRNum tolerance, ctypeRNum step_size)
    {
        grampc_check_gradients(grampc_, tolerance, step_size);
    }

    void Grampc::get_config_from_file(const typeChar *fileName)
    {
        grampc_get_config_from_file(grampc_, fileName);
    }
