...
int main()
{
    // Definition of the kernel for both GPs
    StationaryKernelPtr kernel = SquaredExponential(2, 1, {3, 3});

    // Definition of the GPs for the dynamics of x1 and x2. Both GPs depend on x2 and u but not on x1
    GaussianProcessPtr gp1 = GP("GP1.txt", kernel, {false, true}, {true});
    GaussianProcessPtr gp2 = GP("GP2.txt", kernel, {false, true}, {true});
   ...
   // State distribution
    ...
    ProblemDescriptionPtr integratorProblem = ProblemDescriptionPtr(new DoubleIntegratorGPProblemDescription(pCost, pCon));

    // configure stochastic problem description
    ResamplingGPProblemDescriptionPtr problem = ResamplingGPProblem(integratorProblem, constraintApprox, transform, {gp1, gp2}, {0, 1});
