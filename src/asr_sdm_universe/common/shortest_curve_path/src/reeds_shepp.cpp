#include "shortest_curve_path/reeds_shepp.hpp"
...
double ReedsSheppStateSpace::distance(ReedsSheppStateSpace &traj)
{
    return rho_ * traj.pathInfo.length();//rho_ * reedsShepp(q0, q1).length();
}

ReedsSheppStateSpace::ReedsSheppPath ReedsSheppStateSpace::reedsShepp(double q0[3], double q1[3], ReedsSheppStateSpace &traj)
{
    traj.q0[0] = q0[0]; traj.q0[1] = q0[1]; traj.q0[2] = q0[2];
    traj.q1[0] = q1[0]; traj.q1[1] = q1[1]; traj.q1[2] = q1[2];

    double dx = q1[0] - q0[0], dy = q1[1] - q0[1], dth = q1[2] - q0[2];
    double c = cos(q0[2]), s = sin(q0[2]);
    double x = c*dx + s*dy, y = -s*dx + c*dy;

    traj.pathInfo = ::reedsShepp(x/rho_, y/rho_, dth);
    return traj.pathInfo; //::reedsShepp(x/rho_, y/rho_, dth);
}
...
void ReedsSheppStateSpace::sample(ReedsSheppStateSpace &traj, double dist, double qnew[3])//, ReedsSheppPathSamplingCallback cb, void* user_data)
{
    // ReedsSheppPath path = reedsShepp(q0, q1);
    // double length = rho_ * traj.pathInfo.length();

    if ((dist <= traj.pathInfo.length())&&(dist >= 0)){
        interpolate(traj.q0, traj.pathInfo, dist/rho_, qnew);
        // cb( qnew, user_data);
    }
    return;
}
