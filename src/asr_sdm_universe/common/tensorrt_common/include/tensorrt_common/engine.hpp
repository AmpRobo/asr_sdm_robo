...
// TensorRT 11 uses strongly typed networks: FP16/INT8 must be present in the ONNX
// (e.g. via NVIDIA ModelOpt AutoCast / quantization). BuilderFlag::kFP16/kINT8 were removed.
...
struct Options {
    // Preferred precision tag (used for engine naming / validation messages).
    // For FP16/INT8, preprocess the ONNX offline; TensorRT 11 will not auto-cast.
    Precision precision = Precision::FP16;
    // Kept for API compatibility; unused in TensorRT 11 (quantize ONNX offline instead).
    std::string calibrationDataDirectoryPath;
    // Kept for API compatibility; unused in TensorRT 11.
