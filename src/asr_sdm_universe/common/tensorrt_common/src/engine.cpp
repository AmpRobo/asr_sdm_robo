...
bool Engine::build(std::string onnxModelPath, const std::array<float, 3>& subVals, const std::array<float, 3>& divVals,
    ...
    if (!doesFileExist(onnxModelPath)) {
        throw std::runtime_error("Could not find ONNX model file at path: " + onnxModelPath);
    }

    // Was not able to find the engine file, generate...
    std::cout << "Engine not found, generating. This could take a while..." << std::endl;

    // Create the TensorRT builder
    auto builder = std::unique_ptr<nvinfer1::IBuilder>(nvinfer1::createInferBuilder(m_logger));
    if (!builder) {
        return false;
    }

    // TensorRT 11: networks are always explicit-batch + strongly typed. Pass flags=0.
    ...
    // parseFromFile resolves external weight files (e.g. *.onnx.data) relative to the ONNX path.
    auto parsed = parser->parseFromFile(
        onnxModelPath.c_str(), static_cast<int>(ILogger::Severity::kWARNING));
    if (!parsed) {
        std::cout << "Failed to parse ONNX file: " << onnxModelPath << std::endl;
        for (int32_t i = 0; i < parser->getNbErrors(); ++i) {
            std::cout << "  ONNX parse error: " << parser->getError(i)->desc() << std::endl;
        }
        return false;
    }
    ...
    // TensorRT 11 removed BuilderFlag::kFP16 / kINT8 and IBuilderConfig::setInt8Calibrator.
    // Precision must be encoded in the ONNX (e.g. ModelOpt AutoCast / quantization).
    if (m_options.precision == Precision::FP16) {
        std::cout << "Precision tag: FP16. "
                     "Ensure the ONNX was converted with ModelOpt AutoCast "
                     "(python -m modelopt.onnx.autocast --onnx_path model.onnx). "
                     "TensorRT 11 will not auto-cast FP32 ONNX to FP16."
                  << std::endl;
    } else if (m_options.precision == Precision::INT8) {
        throw std::runtime_error(
            "INT8 is not supported via builder calibrator in TensorRT 11. "
            "Quantize the ONNX offline with ModelOpt, then build with Precision::FP32/FP16 tag "
            "matching the quantized model types.");
    } else {
        std::cout << "Precision tag: FP32 (types taken from ONNX)." << std::endl;
    }
...
bool Engine::loadNetwork(std::string onnxModelPath) {
    ...
    if (!doesFileExist(engine_path)) {
        throw std::runtime_error("Unable to find engine file at path: " + engine_path);
    }
    std::ifstream file(engine_path, std::ios::binary | std::ios::ate);
    if (!file) {
        throw std::runtime_error("Unable to open engine file: " + engine_path);
    }
    const auto size = file.tellg();
    if (size <= 0) {
        throw std::runtime_error("Engine file is empty or unreadable: " + engine_path);
    }
...
bool Engine::runInference(const std::vector<std::vector<cv::cuda::GpuMat>> &inputs, std::vector<std::vector<std::vector<float>>>& featureVectors) {
    ...
    for (int batch = 0; batch < batchSize; ++batch) {
        ...
        for (int32_t outputBinding = static_cast<int32_t>(numInputs); outputBinding < m_engine->getNbIOTensors(); ++outputBinding) {
