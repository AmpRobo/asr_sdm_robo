...
public:
  ...
  void initialize()
  {
    ...
    auto odometry_visualization_qos = rclcpp::QoS(rclcpp::KeepLast(1));
    odometry_visualization_qos.reliable();
    odometry_visualization_qos.transient_local();
    odometry_visualization_pub_ =
      create_publisher<visualization_msgs::msg::MarkerArray>(
        "/map/esdf_map/odometry_visualization", odometry_visualization_qos);
    auto odometry_qos = rclcpp::SensorDataQoS();
    odometry_qos.keep_last(10);
    odometry_visualization_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      odom_topic_, odometry_qos,
      std::bind(
        &ESDFMapTestNode::odometryVisualizationCallback, this,
        std::placeholders::_1));
...
private:
  ...
  void odometryVisualizationCallback(
    const nav_msgs::msg::Odometry::ConstSharedPtr odom)
  {
    ...
    const std::string frame_id = odom->header.frame_id.empty() ?
      get_parameter("test.frame_id").as_string() : odom->header.frame_id;
    if (!odometry_frame_id_.empty() && odometry_frame_id_ != frame_id) {
      odometry_path_.clear();
    }
    odometry_frame_id_ = frame_id;

    const geometry_msgs::msg::Point current_point = pointMsg(position);
    if (odometry_path_.empty()) {
      odometry_path_.push_back(current_point);
    } else {
      const geometry_msgs::msg::Point & last_point = odometry_path_.back();
      const Eigen::Vector3d last_position(last_point.x, last_point.y, last_point.z);
      if ((position - last_position).norm() > 1e-3) {
        odometry_path_.push_back(current_point);
      }
    }
    ...
    path.color.b = 0.0F;
    path.color.a = 1.0F;
    path.points = odometry_path_;
    markers.markers.push_back(std::move(path));

    const Eigen::Matrix3d rotation = orientation.toRotationMatrix();
    constexpr double axis_length = 0.8;
    ...
    for (int axis = 0; axis < 3; ++axis) {
      ...
      marker.scale.z = 0.15;
      marker.color.a = 1.0F;
      marker.color.r = axis == 0 ? 1.0F : 0.0F;
      marker.color.g = axis == 1 ? 1.0F : 0.0F;
      marker.color.b = axis == 2 ? 1.0F : 0.0F;
      marker.points.push_back(current_point);
      marker.points.push_back(pointMsg(position + axis_length * axes[axis]));
      markers.markers.push_back(std::move(marker));
    }

    odometry_visualization_pub_->publish(markers);
  }
  ...
  std::vector<geometry_msgs::msg::Point> odometry_path_;
  std::string odometry_frame_id_;

  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr odometry_visualization_sub_;
