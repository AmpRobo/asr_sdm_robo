...
#include "asr_sdm_esdf_map/ESDFMap.hpp"
#include "asr_sdm_esdf_map/raycast.hpp"
#include "asr_sdm_esdf_map/timing.hpp"
#include <iostream>
...
class Fiesta {
private:
    using DepthMsg = std::remove_const_t<typename DepthMsgType::element_type>;
    using PoseMsg = std::remove_const_t<typename PoseMsgType::element_type>;
...
#endif
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr slice_pub_, text_pub_;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud>::SharedPtr occupancy_pub_;
    typename rclcpp::Subscription<PoseMsg>::SharedPtr transform_sub_;
    typename rclcpp::Subscription<DepthMsg>::SharedPtr depth_sub_;
...
public:
    Fiesta(const rclcpp::Node::SharedPtr &node);
    ~Fiesta();
};

template<class DepthMsgType, class PoseMsgType>
Fiesta<DepthMsgType, PoseMsgType>::Fiesta(const rclcpp::Node::SharedPtr &node) : node_(node) {
...
#endif
    transform_sub_ = node_->create_subscription<PoseMsg>(
        "transform", 10, std::bind(&Fiesta::PoseCallback, this, std::placeholders::_1));
    depth_sub_ = node_->create_subscription<DepthMsg>(
     ...
     slice_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
         "ESDFMap/slice", rclcpp::QoS(1).transient_local());
     occupancy_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud>(
         "ESDFMap/occ_pc", rclcpp::QoS(1).transient_local());
     text_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
         "ESDFMap/text", rclcpp::QoS(1).transient_local());
...
void Fiesta<DepthMsgType, PoseMsgType>::Visualization(ESDFMap *esdf_map, bool global_vis, const std::string &text) {
     if (esdf_map!=nullptr) {
          if (global_vis) esdf_map->SetOriginalRange();
          else esdf_map->SetUpdateRange(cur_pos_ - parameters_.radius_, cur_pos_ + parameters_.radius_, false);
...
void Fiesta<DepthMsgType, PoseMsgType>::RaycastProcess(int i, int part, int tt) {
     ...
     for (int idx = part*i; idx < part*(i + 1); idx++) {
          ...
          if (idx >= static_cast<int>(cloud_.points.size())) break;
          ...
          if (length < parameters_.min_ray_length_) continue;
          ...
          } else tmp_idx = esdf_map_->SetOccupancy((Eigen::Vector3d) point, 1);
          if (tmp_idx!=-10000) {
#ifdef HASH_TABLE
               if (set_occ_.find(tmp_idx) != set_occ_.end()) continue;
               else set_occ_.insert(tmp_idx);
#else
               if (set_occ_[tmp_idx]==tt) continue; else set_occ_[tmp_idx] = tt;
#endif
          }
          Raycast(raycast_origin_/parameters_.resolution_, point/parameters_.resolution_,
                  parameters_.l_cornor_/parameters_.resolution_, parameters_.r_cornor_/parameters_.resolution_, &output);
          for (int i = output.size() - 2; i >= 0; i--) {
               Eigen::Vector3d tmp = (output[i] + half)*parameters_.resolution_;
               length = (tmp - raycast_origin_).norm();
               if (length < parameters_.min_ray_length_) break;
               if (length > parameters_.max_ray_length_) continue;
               tmp_idx = esdf_map_->SetOccupancy(tmp, 0);
               if (tmp_idx!=-10000) {
#ifdef HASH_TABLE
                    if (set_free_.find(tmp_idx) != set_free_.end()) break;
                    else set_free_.insert(tmp_idx);
#else
                    if (set_free_[tmp_idx]==tt) break;
                    else set_free_[tmp_idx] = tt;
#endif
               }
          }
     }
...
void Fiesta<DepthMsgType, PoseMsgType>::RaycastMultithread() {
     ...
     if (parameters_.ray_cast_num_thread_==0) {
          ...
          for (int i = 0; i < parameters_.ray_cast_num_thread_; ++i) {
...
void Fiesta<DepthMsgType, PoseMsgType>::DepthConversion() {
     ...
     cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(depth_queue_.front(), depth_queue_.front()->encoding);
     ...
     if (!parameters_.use_depth_filter_) {
          ...
          for (int v = parameters_.depth_filter_margin_; v < rows - parameters_.depth_filter_margin_; v++) {
               ...
               for (int u = parameters_.depth_filter_margin_; u < cols - parameters_.depth_filter_margin_; u++) {
                    ...
                    point.y = (v - parameters_.center_y_)*depth/parameters_.focal_y_;
                    point.z = depth;
                    if (depth > parameters_.depth_filter_max_dist_ || depth < parameters_.depth_filter_min_dist_) continue;
...
void Fiesta<DepthMsgType, PoseMsgType>::SynchronizationAndProcess() {
     ...
     while (!depth_queue_.empty()) {
          ...
          return;
#else
          if (parameters_.use_depth_filter_) last_transform_ = transform_;
          ...
          if constexpr(std::is_same<DepthMsgType, sensor_msgs::msg::Image::ConstSharedPtr>::value) {
               DepthConversion();
          } else if constexpr(std::is_same<DepthMsgType, sensor_msgs::msg::PointCloud2::ConstSharedPtr>::value) {
...
void Fiesta<DepthMsgType, PoseMsgType>::PoseCallback(const PoseMsgType &msg) {
     Eigen::Vector3d pos;
     Eigen::Quaterniond q;
     if constexpr(std::is_same<PoseMsgType, geometry_msgs::msg::PoseStamped::ConstSharedPtr>::value) {
     ...
     } else if constexpr(std::is_same<PoseMsgType, nav_msgs::msg::Odometry::ConstSharedPtr>::value) {
          pos = Eigen::Vector3d(msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z);
          q = Eigen::Quaterniond(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);
     } else if constexpr(std::is_same<PoseMsgType, geometry_msgs::msg::TransformStamped::ConstSharedPtr>::value) {
          pos = Eigen::Vector3d(msg->transform.translation.x, msg->transform.translation.y, msg->transform.translation.z);
          q = Eigen::Quaterniond(msg->transform.rotation.w, msg->transform.rotation.x, msg->transform.rotation.y, msg->transform.rotation.z);
     }
...
void Fiesta<DepthMsgType, PoseMsgType>::UpdateEsdfEvent() {
     ...
     if (esdf_map_->CheckUpdate()) {
          ...
          if (parameters_.global_update_) esdf_map_->SetOriginalRange();
          else esdf_map_->SetUpdateRange(cur_pos_ - parameters_.radius_, cur_pos_ + parameters_.radius_);
