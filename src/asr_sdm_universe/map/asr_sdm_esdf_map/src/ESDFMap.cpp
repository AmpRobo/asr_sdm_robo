...
const double amp::ESDFMap::Logit(const double &x) const {
  return log(x / (1 - x));
}

bool amp::ESDFMap::Exist(const int &idx) {
#ifdef PROBABILISTIC
...
void amp::ESDFMap::DeleteFromList(int link, int idx) {
...
void amp::ESDFMap::InsertIntoList(int link, int idx) {
...
bool amp::ESDFMap::PosInMap(Eigen::Vector3d pos) {
#ifdef HASH_TABLE
  return true;
#else
...
bool amp::ESDFMap::VoxInRange(Eigen::Vector3i vox, bool current_vec) {
  if (current_vec)
...
void amp::ESDFMap::Pos2Vox(Eigen::Vector3d pos, Eigen::Vector3i &vox) {
  for (int i = 0; i < 3; ++i)
    vox(i) = floor((pos(i) - origin_(i)) / resolution_);
}

void amp::ESDFMap::Vox2Pos(Eigen::Vector3i vox, Eigen::Vector3d &pos) {
  for (int i = 0; i < 3; ++i)
    pos(i) = (vox(i) + 0.5) * resolution_ + origin_(i);
}

int amp::ESDFMap::Vox2Idx(Eigen::Vector3i vox) {
#ifdef HASH_TABLE
...
int amp::ESDFMap::Vox2Idx(Eigen::Vector3i vox, int sub_sampling_factor) {
#ifdef HASH_TABLE
...
Eigen::Vector3i amp::ESDFMap::Idx2Vox(int idx) {
#ifdef HASH_TABLE
...
double amp::ESDFMap::Dist(Eigen::Vector3i a, Eigen::Vector3i b) {
...
amp::ESDFMap::ESDFMap(Eigen::Vector3d origin, double resolution_, int reserve_size)
...
amp::ESDFMap::ESDFMap(Eigen::Vector3d origin, double resolution_, Eigen::Vector3d map_size)
    : map_size_(map_size), origin_(origin), resolution_(resolution_) {
  resolution_inv_ = 1 / resolution_;
...
#ifdef PROBABILISTIC
void amp::ESDFMap::SetParameters(double p_hit, double p_miss, double p_min, double p_max, double p_occ) {
...
bool amp::ESDFMap::UpdateOccupancy(bool global_map) {
#ifdef PROBABILISTIC
...
int amp::ESDFMap::SetOccupancy(Eigen::Vector3d pos, int occ) {
  if (occ != 1 && occ != 0) {
...
int amp::ESDFMap::SetOccupancy(Eigen::Vector3i vox, int occ) {
...
int amp::ESDFMap::GetOccupancy(Eigen::Vector3d pos) {
  if (!PosInMap(pos))
    return undefined_;
...
int amp::ESDFMap::GetOccupancy(Eigen::Vector3i pos_id) {
  // TODO: no boundary check
  return Exist(Vox2Idx(std::move(pos_id)));
}

double amp::ESDFMap::GetDistance(Eigen::Vector3d pos) {
  if (!PosInMap(pos))
    return undefined_;
...
double amp::ESDFMap::GetDistance(Eigen::Vector3i vox) {
  return distance_buffer_[Vox2Idx(vox)] < 0 ? infinity_ : distance_buffer_[Vox2Idx(vox)];
}

double amp::ESDFMap::GetDistWithGradTrilinear(Eigen::Vector3d pos,
                                                 Eigen::Vector3d &grad) {
  if (!PosInMap(pos))
    return -1;
...
void amp::ESDFMap::GetPointCloud(sensor_msgs::msg::PointCloud &m, int vis_lower_bound, int vis_upper_bound) {
...
void amp::ESDFMap::GetSliceMarker(visualization_msgs::msg::Marker &m, int slice, int id,
...
// region HASH_TABLE
#ifdef HASH_TABLE
void amp::ESDFMap::IncreaseCapacity(int &old_size, int new_size) {
...
int amp::ESDFMap::FindAndInsert(Eigen::Vector3i hash_key) {
#ifdef BLOCK
...
void amp::ESDFMap::SetUpdateRange(Eigen::Vector3d min_pos, Eigen::Vector3d max_pos, bool new_vec) {
#ifndef HASH_TABLE
...
void amp::ESDFMap::SetOriginalRange() {
#ifdef HASH_TABLE
...
bool amp::ESDFMap::CheckConsistency() {
#ifdef HASH_TABLE
...
bool amp::ESDFMap::CheckWithGroundTruth() {
#ifdef HASH_TABLE
