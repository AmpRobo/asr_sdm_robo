...
#include <logging_simulator/Quadrotor.h>
...
main(int argc, char** argv)
{
  ...
  auto n = rclcpp::Node::make_shared("logging_simulator", options);
  ...
  n->get_parameter_or("simulator.init_state_x", _init_x, 0.0);
  n->get_parameter_or("simulator.init_state_y", _init_y, 0.0);
  n->get_parameter_or("simulator.init_state_z", _init_z, 1.0);

  Eigen::Vector3d position = Eigen::Vector3d(_init_x, _init_y, _init_z);
  quad.setStatePos(position);

  double simulation_rate;
  n->get_parameter_or("rate.simulation", simulation_rate, 1000.0);
  assert(simulation_rate > 0);

  double odom_rate;
  n->get_parameter_or("rate.odom", odom_rate, 100.0);
  const rclcpp::Duration odom_pub_duration = rclcpp::Duration::from_seconds(1 / odom_rate);

  std::string quad_name;
  n->get_parameter_or("quadrotor_name", quad_name, std::string("quadrotor"));
