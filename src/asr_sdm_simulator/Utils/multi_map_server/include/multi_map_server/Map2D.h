...
namespace multi_map_server_util
{
...
public:
  ...
  void Update(nav_msgs::msg::OccupancyGrid m)
  {
    ...
    double oyaw = multi_map_server_util::getYaw(m.info.origin.orientation);
  ...
  {
    rclcpp::Clock clock;
    map.header.stamp       = clock.now();
    map.info.map_load_time = clock.now();
